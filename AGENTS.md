...
- Azure Pipelines: optional redundant CPU verification (see `azure-pipelines.yml`, GH#109). Free-tier posture: self-hosted Linux in the Default pool on **main pushes only** (`pr: none` — PR validation stays on GitHub Actions so PR-authored YAML cannot schedule the self-hosted pool). Microsoft-hosted 3-OS matrix is commented out for re-enable when free minutes return. Runs fmt/clippy/test/check/llvm-cov (CPU-only; GPU stays on GHA). When using Azure, keep the self-hosted agent listening (operator start command is documented in `azure-pipelines.yml`); if the agent is offline, jobs queue until it comes up, or rely on GHA alone. Results may not appear in the GitHub Actions UI unless the Azure Pipelines GitHub App reports statuses. Prefer azure-devops MCP tools (e.g. pipelines_get_build_status, list_runs) for authoritative Azure state. Note: /scripts/* (incl. sentry_release.sh) are deliberately gitignored (chore d1d02c8) for local-only use; GHA health steps use echo + MCP for PRs.
