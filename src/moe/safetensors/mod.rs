...
// Keep discovery helpers in parent scope so `tests.rs` (`use super::*`) resolves them.
#[allow(unused_imports)]
use discovery::{classify_tensor, discover_candidates};
// Glob-import so tests and siblings keep resolving extracted helpers.
use json::*;
use manifest::*;
use paths::*;
use validate::*;
...
// Crate-private helper used by `moe/tests.rs`.
pub(crate) use validate::dtype_size_bytes;

// Used by map backend (header size checks when probing shards).
use manifest::MAX_HEADER_BYTES;
